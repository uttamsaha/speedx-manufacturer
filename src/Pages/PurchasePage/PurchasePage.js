...
const PurchasePage = () => {
  ...
  const [error, setError] = useState('');
    ...
    const handleQuantity = (event) => {
      ...
      if(quantity >= tool?.minOrderQuantity && quantity <= tool?.available){
        ...
        setTrue(false);
        setNewQuantity(0);
        setError(`Minimum Order ${tool.minOrderQuantity} and Maximum Order ${tool?.available}`)
      }
    }
  ...
  return (
    <div className='flex justify-center gap-8 mt-8'>
      ...
      <input type="number" className='input input-bordered w-full max-w-lg mb-3' onChange={handleQuantity}  placeholder="Enter quantity" required/> 
      ...
      <input className='btn btn-primary text-white lg:w-96 xs:w-xs' type="submit" value="Place Order" disabled={!isTrue} />
    </form>
    </div>
  )
}
